...
import xyz.monkeytong.hongbao.utils.SPUtil;
...
public class HongbaoService extends AccessibilityService implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    public void onAccessibilityEvent(AccessibilityEvent event) {
        Log.i("SKY", "onAccessibilityEvent,...EventType is " + (null == event ? "null" : event.getEventType()));
        ...
        if (!mMutex) {
            //是否 监视系统通知
            if (sharedPreferences.getBoolean(SPUtil.pref_watch_notification, false) && watchNotifications(event)) {
                return;
            }
            //是否 监视聊天列表
    ...
    private void setCurrentActivityName(AccessibilityEvent event) {
        ...
        try {
            ...
            Log.i("SKY", "setCurrentActivityName,...PackageName is " + event.getPackageName().toString());
            Log.i("SKY", "setCurrentActivityName,...ClassName is " + event.getClassName().toString());
            getPackageManager().getActivityInfo(componentName, 0);
            currentActivityName = componentName.flattenToShortString();
            Log.i("SKY", "setCurrentActivityName,...currentActivityName is " + currentActivityName);
    ...
    private boolean watchNotifications(AccessibilityEvent event) {
        ...
        Log.e("SKY", "watchNotifications,...tip is " + tip);
    ...
    private void watchFlagsFromPreference() {
        ...
        Log.i("SKY", "watchFlagsFromPreference, watchOnLockFlag = " + watchOnLockFlag);
        this.powerUtil.handleWakeLock(watchOnLockFlag);
    }

    @Override
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        Log.i("SKY", "onSharedPreferenceChanged, key = " + key);
